...
-- Execute this file to set up both frontier_reviews tables from scratch
-- =============================================================================
...
-- =============================================================================
-- STEP 1: Create frontier_reviews (Raw/Bronze Layer)
-- =============================================================================

-- Drop existing table if you want a fresh start (CAUTION: destroys data!)
-- Uncomment the line below only if you're sure you want to drop existing data
...
CREATE TABLE IF NOT EXISTS frontier_reviews (
    ...
    -- Unique review identifier from source platform
...
CREATE INDEX IF NOT EXISTS idx_raw_review_id ON frontier_reviews(review_id);
CREATE INDEX IF NOT EXISTS idx_raw_platform ON frontier_reviews(platform);
CREATE INDEX IF NOT EXISTS idx_raw_is_processed ON frontier_reviews(is_processed);
CREATE INDEX IF NOT EXISTS idx_raw_review_date ON frontier_reviews(review_date DESC);

-- Trigger for updated_at
CREATE OR REPLACE FUNCTION update_raw_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
DROP TRIGGER IF EXISTS trigger_update_raw_timestamp ON frontier_reviews;
CREATE TRIGGER trigger_update_raw_timestamp
    BEFORE UPDATE ON frontier_reviews
    FOR EACH ROW
    EXECUTE FUNCTION update_raw_updated_at_column();

-- Comments
COMMENT ON TABLE frontier_reviews IS 'Bronze layer: Raw review data as ingested from source platforms';
COMMENT ON COLUMN frontier_reviews.is_processed IS 'Flag indicating if review has been processed by AI pipeline';

-- =============================================================================
-- STEP 2: Create frontier_reviews_processed (Processed/Silver Layer)
-- =============================================================================

CREATE TABLE IF NOT EXISTS frontier_reviews_processed (
    -- Primary identifier
    id SERIAL PRIMARY KEY,
    
    -- Foreign key to original review
    review_id INTEGER NOT NULL UNIQUE REFERENCES frontier_reviews(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_processed_review_id ON frontier_reviews_processed(review_id);
CREATE INDEX IF NOT EXISTS idx_processed_status ON frontier_reviews_processed(processing_status);
CREATE INDEX IF NOT EXISTS idx_processed_sentiment_score ON frontier_reviews_processed(sentiment_score);
CREATE INDEX IF NOT EXISTS idx_processed_churn_risk ON frontier_reviews_processed(churn_risk);
CREATE INDEX IF NOT EXISTS idx_processed_primary_category ON frontier_reviews_processed(primary_category);
CREATE INDEX IF NOT EXISTS idx_processed_review_date ON frontier_reviews_processed(review_date DESC);
CREATE INDEX IF NOT EXISTS idx_processed_platform ON frontier_reviews_processed(platform);

-- Indexes for geographic metadata
CREATE INDEX IF NOT EXISTS idx_processed_state ON frontier_reviews_processed(state);
CREATE INDEX IF NOT EXISTS idx_processed_city ON frontier_reviews_processed(city);
CREATE INDEX IF NOT EXISTS idx_processed_area_type ON frontier_reviews_processed(area_type);
CREATE INDEX IF NOT EXISTS idx_processed_region ON frontier_reviews_processed(region);

-- Indexes for temporal metadata
CREATE INDEX IF NOT EXISTS idx_processed_year_month ON frontier_reviews_processed(year, month);
CREATE INDEX IF NOT EXISTS idx_processed_quarter ON frontier_reviews_processed(quarter);
CREATE INDEX IF NOT EXISTS idx_processed_year ON frontier_reviews_processed(year);

-- GIN index for JSONB queries on ai_attributes
CREATE INDEX IF NOT EXISTS idx_processed_ai_attributes_gin ON frontier_reviews_processed USING gin(ai_attributes);

-- HNSW index for vector similarity search (pgvector)
CREATE INDEX IF NOT EXISTS idx_processed_gte_embedding_hnsw 
...
DROP TRIGGER IF EXISTS trigger_update_processed_timestamp ON frontier_reviews_processed;
...
-- DONE! 
-- =============================================================================
-- Your database is now ready for the n8n workflow to process reviews.
-- Next steps:
-- 1. Import your raw review data into frontier_reviews table
-- 2. Run the nimbus_ai_processor.json workflow in n8n
-- =============================================================================
